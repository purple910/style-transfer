...
image_size = 256
batch_size = 4
content_batch = get_content_tfrecord(batch_size, os.path.join('data', 'train2014.tfrecords'), image_size)
...
def load_net(data_path):
    ...
    vgg_net = data['layers'][0]
    return vgg_net, mean_pixel


vgg_layers, mean_pixel = load_net('vggnet/imagenet-vgg-verydeep-19.mat')
...
def vgg_endpoints(inputs, reuse=None):
    with tf.variable_scope('endpoints', reuse=reuse):
        def _weights(layer, expected_layer_name):
            ...
            layer_name = vgg_layers[layer][0][0][0][0]
...
OUTPUT_DIR = 'models/vgg19_tf_train2014_' + style_name
...
X_sample = imread('./content/chicago.jpg')
...
for e in range(10000):
    ...
    X_batch = np.reshape(batch_content, [batch_size, image_size, image_size, 3])
    # 使用X_batch 替换掉X
    ls_, _ = sess.run([loss, optimizer], feed_dict={X: X_batch})

    if e % 100 == 0:
        print('Epoch %d Loss %f' % (e, ls_))
