...
# 风格图像张数(./style/png/)
parser.add_argument("--LABELS_NUMS", type=int, default=26)
# Batch大小，默认为2
...
def backward(img_h=args.IMG_H, img_w=args.IMG_W, img_c=args.IMG_C, style_h=args.STYLE_H, style_w=args.STYLE_W,
    ...
    with tf.Session() as sess:
        ...
        for itr in range(args.steps):
            ...
            # if itr % 100 == 0:
            #     # 为之后打印信息进行相关计算
            #     [loss_p, target_p, content_loss_res, style_loss_res] = sess.run(
            #         [loss, target, content_loss, style_loss],
            ...
            #         "result/" + str(step) + "_" + str(np.argmax(y_labels[0, :])) + ".jpg")
            #
            ...
            if step == 50000:
                saver.save(sess, model_path + "model.ckpt-done")
                break
