...
class Stylizer(object):
    ...
    def save_result(self):
        ...
        self.img25_4.show()
        self.img1.save(self.stylizer_arg.PATH_RESULTS + self.img_path.split('/')[-1].split('.')[0] + '_' + str(
            self.label_list) + '_result_1' + '.jpg')
        self.img1.show()
        print('Image Matrix Saved Successfully!')

    def generate_one(self):
        img_return = self.stylize(
            [self.stylizer_arg.ALPHA1, self.stylizer_arg.ALPHA2, self.stylizer_arg.ALPHA3, self.stylizer_arg.ALPHA4])
        img_return = Image.fromarray(np.uint8(img_return[0, :, :, :]))
        width, height = img_return.size
